...
export function TaskDashboardModal({ isOpen, onClose, tasks, employees, onAddTask, onUpdateTask, onDeleteTask, addLog }: TaskDashboardModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-ink/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm font-mono flex-col">
      <div className="bg-white border-4 border-ink w-full max-w-5xl shadow-[8px_8px_0_0_#fff] flex flex-col max-h-[85vh]">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          {isAdding && (
            <form onSubmit={handleAdd} className="p-4 bg-white border-b-2 border-ink grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-4 items-end shrink-0">
          ...
          <div className="flex-1 overflow-y-auto bg-paper p-4 grid gap-3">
            {tasks.length === 0 ? (
              <div className="text-center p-12 text-ink/40 font-bold uppercase text-sm tracking-widest flex items-center justify-center italic">
                No directives found
              </div>
            ) : (
              tasks.sort((a,b) => new Date(a.deadline).getTime() - new Date(b.deadline).getTime()).map(task => {
                ...
                return (
                  <div key={task.id} className={`bg-white border-2 border-ink flex flex-col sm:flex-row shadow-[4px_4px_0_0_#141414] ${overdue ? 'border-offline-state shadow-[4px_4px_0_0_#ff3333]' : ''}`}>
                    ...
                    <div className="p-3 flex-1 flex flex-col justify-center">
                      ...
                      <h3 className={`font-bold leading-tight ${task.status === 'DONE' ? 'line-through opacity-50' : ''}`}>{task.title}</h3>
                    </div>

                    <div className="p-3 border-t-2 sm:border-t-0 sm:border-l-2 border-ink/20 flex flex-col justify-center bg-paper text-xs sm:w-48 shrink-0">
                    ...
                    <div className="p-3 border-t-2 sm:border-t-0 sm:border-l-2 border-ink/20 flex flex-col justify-center bg-paper text-xs sm:w-48 shrink-0 relative">
